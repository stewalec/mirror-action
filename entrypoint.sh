...
if [[ "${GIT_SSH_PRIVATE_KEY}" != "" ]]; then
    ...
    if [[ "${GIT_SSH_KNOWN_HOSTS}" != "" ]]; then
      ...
      git config --global core.sshCommand "ssh -vvv -i /github/home/.ssh/id_rsa -o IdentitiesOnly=yes -o UserKnownHostsFile=/github/home/.ssh/known_hosts"
    else
      if [[ "${GIT_SSH_NO_VERIFY_HOST}" != "true" ]]; then
        ...
        git config --global core.sshCommand "ssh -vvv -i /github/home/.ssh/id_rsa -o IdentitiesOnly=yes -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no"
      fi
    fi
else
