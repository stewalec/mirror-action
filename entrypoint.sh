...
if [[ "${GIT_SSH_PRIVATE_KEY}" != "" ]]; then
    ...
    if [[ "${GIT_SSH_KNOWN_HOSTS}" != "" ]]; then
      ...
      git config --global core.sshCommand "ssh -vvv -o IdentityAgent=$SSH_AUTH_SOCK -o IdentitiesOnly=no -o UserKnownHostsFile=/github/home/.ssh/known_hosts"
    else
      if [[ "${GIT_SSH_NO_VERIFY_HOST}" != "true" ]]; then
        ...
        git config --global core.sshCommand "ssh -vvv -o IdentityAgent=$SSH_AUTH_SOCK -o IdentitiesOnly=no -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no"
      fi
    fi
else
